...
class MultiModalDataset(Dataset):
    def __init__(self, model, data, data_arguments, mode, loaded_images=None):
        super(MultiModalDataset, self).__init__()
        self.data_arguments = data_arguments
        self.mode = mode
        self.data = data
        self.loaded_images = loaded_images if loaded_images is not None else {}
        ...
        if "image" in data_item:  # for multi image
            ...
            for filename in data_item["image"]:
                if filename in self.loaded_images:
                    # 如果图像已预加载，直接使用
                    image = self.loaded_images[filename]
                else:
                    # 否则，按原方式加载
